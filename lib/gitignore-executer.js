...
export function getMatches(files,gitignorePath){
    ...
    const gitignoreList = Object.values(gitignore(fs.readFileSync(gitignorePath).toString())).flat(1).map(value => typeof value == "object" ? ("name" in value ? value.patterns : undefined) : value).flat(1);
    const matches = micromatch(files,gitignoreList);
    console.log(files, gitignoreList,matches)
    return matches;
}
/**
 * 
